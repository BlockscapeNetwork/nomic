...
use nomic::chain::client::{Client as PegClient};
use nomic::core::primitives::transaction::{HeaderTransaction, Transaction};
...
pub fn make_rpc_client() -> bitcoincore_rpc::Result<Client> {
    let rpc_user = env::var("BTC_RPC_USER").unwrap();
    let rpc_pass = env::var("BTC_RPC_PASS").unwrap();

    debug!("User: {}, Pass: {}", rpc_user, rpc_pass);
